...
public class PAPISupport extends PlaceholderExpansion {
    ...
    public String onPlaceholderRequest(Player player, @NotNull String s) {
        ...
        switch (s) {
            case "player_kills":
                return (arena != null) ? String.valueOf(arena.getPlayerKills(player, false)) : "";
            case "player_kills_total":
                return (arena != null) ? String.valueOf(arena.getPlayerKills(player, true) + arena.getPlayerKills(player, false)) : "";
            case "player_kills_final":
                return (arena != null) ? String.valueOf(arena.getPlayerKills(player, true)) : "";
            case "player_deaths":
                return (arena != null) ? String.valueOf(arena.getPlayerDeaths(player, false)) : "";
            case "player_deaths_total":
                return (arena != null) ? String.valueOf(arena.getPlayerDeaths(player, true) + arena.getPlayerDeaths(player, false)) : "";
            case "player_deaths_final":
                return (arena != null) ? String.valueOf(arena.getPlayerDeaths(player, true)) : "";
            case "player_beds":
                return (arena != null) ? String.valueOf(arena.getPlayerBedsDestroyed(player)) : "";
            case "status_color":
                return (arena != null) ? (arena.isSpectator(player) ? ChatColor.GRAY : arena.getTeam(player).getColor().chat()).toString() : "";
            case "status_letter":
                return (arena != null) ? arena.isSpectator(player) ? getSpectatorLetter(lang) : arena.getTeam(player).getName().substring(0, 1).toUpperCase() : "";
        }
