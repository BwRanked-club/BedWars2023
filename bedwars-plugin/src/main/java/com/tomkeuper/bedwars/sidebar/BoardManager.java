...
public class BoardManager implements IScoreboardService {
    ...
    public List<Scoreboard> registerArenaScoreboards(IArena arena) {
        ...
        for (Language language : Language.getLanguages()) {
            ...
            String scoreboardWaitingName = "bw_" + arena.getGroup() + "_waiting_" + language.getIso() + arenaSuffix;
            ...
            String scoreboardStartingName = "bw_" + arena.getGroup() + "_starting_" + language.getIso() + arenaSuffix;
            ...
            String scoreboardPlayingName = "bw_" + arena.getGroup() + "_playing_" + language.getIso() + arenaSuffix;
        ...
        Bukkit.getScheduler().runTaskLater(BedWars.plugin, () -> {
            ...
            if (arenaStatus == null) {
                ...
                scoreboardName = switch (arenaStatus) {
                    case waiting -> "bw_" + arena.getGroup() + "_waiting_" + playerLanguage.getIso() + arenaSuffix;
                    case starting -> "bw_" + arena.getGroup() + "_starting_" + playerLanguage.getIso() + arenaSuffix;
                    case playing, restarting ->
                            "bw_" + arena.getGroup() + "_playing_" + playerLanguage.getIso() + arenaSuffix;
                };
            }
    ...
    private String getTeamPlaceholder(Player player, int teamNumber) {
        ...
        if (arena.getTeams().size() >= teamNumber) {
            return genericTeamFormat
                    .replace("%bw_team_letter%", String.valueOf(!teamName.isEmpty() ? teamName.charAt(0) : ""))
    ...
    public void remove(@NotNull Player player) {
        ...
        try {
            ...
            if (tp == null) return;
            scoreboardManager.resetScoreboard(tp);
        } catch (Throwable ignored) {
        }
    }
        ...
        if (type.equalsIgnoreCase("tab")) {
            ...
            if (arena == null) {
                ...
                if (arena.isSpectator(player)) {
                    ...
                    fixList = switch (arenaStatus) {
                        case playing -> Language.getList(player, Messages.FORMATTING_SCOREBOARD_HEAD_PREFIX_PLAYING);
                        case waiting -> Language.getList(player, Messages.FORMATTING_SCOREBOARD_HEAD_PREFIX_WAITING);
                        case starting -> Language.getList(player, Messages.FORMATTING_SCOREBOARD_HEAD_PREFIX_STARTING);
                        case restarting ->
                                Language.getList(player, Messages.FORMATTING_SCOREBOARD_HEAD_PREFIX_RESTARTING);
                        default -> {
                            BedWars.debug("Unhandled game state for BedWars prefix");
    ...
    public @Nullable Scoreboard getScoreboard(@NotNull Player player) {
        ...
        try {
            return scoreboardManager.getActiveScoreboard(tp);
        } catch (Throwable ignored) {
            return null;
        }
    }
