...
public interface IPlayerQuickBuyCache {
    ...
    /**
     * Exporta o layout atual no formato slot->identifier (somente preenchidos).
     */
    HashMap<Integer, String> exportLayout();

    /**
     * Aplica um layout completo (slot->identifier). Null/blank limpa slot.
     */
    void applyLayout(Map<Integer, String> layout);

    /**
     * Representação compacta (Base64/CSV) para persistência/transporte.
     */
    String serializeCompact();

    /**
     * Aplica um layout a partir de uma string compacta.
     */
    void applyCompact(String compact);
}
